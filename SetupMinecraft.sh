...
Update_Service() {
  ...
  sudo curl -H "Accept-Encoding: identity" -L -o /etc/systemd/system/$ServerName.service https://raw.githubusercontent.com/0x233233/MinecraftBedrockServer/master/minecraftbe.service
  ...
  if command -v apt-get &>/dev/null; then
    ...
    if [[ "$SSLVer" ]]; then
      ...
      if [[ "$CPUArch" == *"x86_64"* ]]; then
        ...
        sudo curl -o libssl.deb -k -L https://github.com/0x233233/Legendary-Bedrock-Container/raw/main/libssl1-1.deb
...
Check_Architecture() {
  ...
  if [[ "$CPUArch" == *"aarch"* ]]; then
    ...
    curl -H "Accept-Encoding: identity" -L -o depends.zip https://raw.githubusercontent.com/0x233233/MinecraftBedrockServer/master/depends.zip
    unzip depends.zip
    sudo mkdir /lib64
